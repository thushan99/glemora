...
const UserManagement = () => {
    ...
    const handleRoleChange = async (userId, newRole) => {
        try {
            ...
            const updatedUsers = users.map(user => user.id === userId ? {...user, roles: [{name: newRole}]} : user);
    ...
    const handleEditUser = () => {
        ...
        setEditedUser({
            ...selectedUser, profilePic: selectedUser.profilePic || null
        });
    };
    ...
    const handleCreateUser = () => {
        ...
        setSelectedUser(null);
        setEditedUser({
            name: '', email: '', username: '', password: '', roles: [{name: 'USER'}]
        });
    };
            ...
            if (isCreating) {
                ...
                if (editedUser.id === currentUserDetails?.id || editedUser.username === user?.username) {
                    ...
                    await api.put('/auth/update-me', updateData, {
                        headers: {
                            'X-Api-Version': 'v1', 'Content-Type': 'multipart/form-data'
                        }
                    });
                } else {
    ...
    const generatePDF = () => {
        ...
        // Add title
        doc.setFontSize(18);
        doc.text('User Management Report', 40, 40);
        ...
        // Generate table
        autoTable(doc, {
            head: [['ID', 'Name', 'Username', 'Email', 'Role', 'Created Date']],
            body: users.map(user => [user.id, user.name || 'N/A', user.username, user.email, user.roles?.[0]?.name || 'USER', user.createdAt ? new Date(user.createdAt).toLocaleDateString() : 'N/A']),
            startY: 60,
            theme: 'grid'
        });
        ...
        doc.setFontSize(14);
        doc.text('Summary Statistics', 40, lastY);

        doc.setFontSize(12);
        doc.text(`Total Users: ${users.length}`, 40, lastY + 20);

        const adminCount = users.filter(u => u.roles?.some(r => r.name === 'ADMIN')).length;
        doc.text(`Admin Users: ${adminCount}`, 40, lastY + 40);

        const userCount = users.filter(u => !u.roles?.some(r => r.name === 'ADMIN')).length;
        doc.text(`Regular Users: ${userCount}`, 40, lastY + 60);
    ...
    return (<div className="container mx-auto px-4 py-8">
            ...
            {error && (<div
                    className={`mb-4 p-4 ${error.includes('Failed') ? 'bg-red-100 text-red-700 border border-red-300' : 'bg-green-100 text-green-700 border border-green-300'} rounded`}>
                    {error}
                    <button
            ...
            {currentUserDetails && (<div className="mb-6 bg-white shadow-md rounded-lg p-6">
                    <h2 className="text-xl font-bold mb-4">Currently Logged In</h2>
                    <div className="flex items-center space-x-4">
                        {currentUserDetails.profilePic && (<img
                        ...
                        <div>
                            <div
                                className="font-medium text-lg">{currentUserDetails.name || currentUserDetails.username}</div>
            ...
            <div className="bg-white shadow-md rounded-lg overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    {users.length > 0 ? (users.map((userItem) => {
                            ...
                            return (<tr key={userItem.id} className={isCurrentUser ? "bg-blue-50" : ""}>
            ...
            {(selectedUser || isCreating) && (
                <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 max-w-lg w-full mx-4">
                        ...
                        {(isEditing || isCreating) ? (<div className="space-y-4">
                                <div>
                    ...
                    hover:file:bg-blue-100"
                                    ...
                                    {editedUser?.profilePic instanceof File && (<div className="mt-2">
                                            <p className="text-sm text-gray-600">New file
                                                selected: {editedUser.profilePic.name}</p>
                                        </div>)}
                                </div>
                                ...
                                {isCreating && (<div>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => setEditedUser({
                                                ...editedUser, roles: [{name: e.target.value}]
                                            })}
                                ...
                                {selectedUser?.profilePic && (<div>
                                        <label className="block text-sm font-medium text-gray-700">Profile
                                            Picture</label>
                                        <div className="mt-1">
                                            <img
